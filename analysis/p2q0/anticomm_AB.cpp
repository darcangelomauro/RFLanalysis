...
int main(int argc, char** argv)
{
    ...
    // Open output file 
    string out_filename_A = path + "/observables/anticomm_AB.txt";
    ofstream out_obs_A;
    out_obs_A.open(out_filename_A);

    if(!out_obs_A)
    {
        cerr << "Error: file " + out_filename_A + " could not be opened." << endl;
        return 1;
    }
    ...
    for(const auto& g2 : g2_vec)
    {
        ...
        for(unsigned i=0; i<job_vec.size(); ++i)
        {
            ...
            for(int j=0; j<sm.samples; ++j) 
            {
                ...
                cx_mat acomm_AB = A*B+B*A;
                double temp_A = trace(acomm_AB*acomm_AB).real();
