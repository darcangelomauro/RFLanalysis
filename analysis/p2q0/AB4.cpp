...
int main(int argc, char** argv)
{
    ...
    // Open output file 
    string out_filename_A = path + "/observables/A4.txt";
    string out_filename_B = path + "/observables/B4.txt";
    ofstream out_obs_A;
    ofstream out_obs_B;
    ...
    for(const auto& g2 : g2_vec)
    {
        ...
        for(unsigned i=0; i<job_vec.size(); ++i)
        {
            ...
            for(int j=0; j<sm.samples; ++j) 
            {
                ...
                cx_mat A2 = A*A;
                cx_mat B2 = B*B;
                double temp_A = trace(A2*A2).real();
                double temp_B = trace(B2*B2).real();
