...
int main(int argc, char** argv)
{
    ...
    // Open output file 
    string out_filename_A = path + "/observables/rA3.txt";
    string out_filename_B = path + "/observables/rAB2.txt";
    ofstream out_obs_A;
    ofstream out_obs_B;
    ...
    for(const auto& g2 : g2_vec)
    {
        ...
        for(unsigned i=0; i<job_vec.size(); ++i)
        {
            ...
            for(int j=0; j<sm.samples; ++j) 
            {
                ...
                double temp_A = rho*trace(A*A*A).real();
                double temp_B = rho*trace(A*B*B).real();
